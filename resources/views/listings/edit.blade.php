...
@section('content')
    <x-card class="p-10 max-w-lg mx-auto mt-24">
        <header class="text-center">
            <h2 class="text-2xl font-bold uppercase mb-1">Edit This Gig</h2>
            <p class="mb-4">Edit: {{$listing->title}}</p>
        </header>

        <!--MATERI DATABASE IN LARAVEL - UPDATE Database in Laravel-->
        <!--attribute enctype="multipart/form-data" dipakai kalo ada input type="file" di form-->
        <form method="POST" action="/home/listings/{{$listing->id}}" enctype="multipart/form-data">
        <!--sebenarnya kalo untuk UPDATE data kita mau method-nya itu PUT, tapi ga bisa kita edit begitu saja di atas (attribute method)-->
            @csrf <!--@csrf Directive untuk aman dari pengiriman data melalui web lain (CSRF Protection)-->
            <!--untuk membuat method-nya PUT maka ada method directive di Laravel-->
            @method('PUT')
            <div class="mb-6">
                <label for="company" class="inline-block text-lg mb-2">Company Name</label>
                <!--MATERI VIEW - Kemampuan Old Form-->
            ...
            <div class="mb-6">
                ...
                <!--tag <textarea> tidak memiliki attribute value, maka untuk kasih default value caranya seperti di bawah-->
